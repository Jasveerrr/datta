        ...
        <div className="relative z-10 h-screen flex flex-col items-center justify-center pointer-events-none px-6 text-center">
          <div className="mt-20 flex flex-col items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-6 pointer-events-auto">
              {/* FIX 3: BOOK A REPAIR wired to /quotes */}
              <MagneticButton
                variant="primary"
                className="px-8 py-4 text-lg border border-primary"
                onClick={() => router.push("/quotes")}
              >
                Book a Repair
              </MagneticButton>
              {/* FIX 4: GET INSTANT QUOTE wired to /quotes */}
              <MagneticButton
                variant="outline"
                className="px-8 py-4 text-lg bg-obsidian/50 backdrop-blur-md"
                onClick={() => router.push("/quotes")}
              >
                Get Instant Quote
              </MagneticButton>
            ...
            <div className="mt-16 flex items-center gap-4 text-silver/60">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full border-2 border-obsidian bg-silver/20 bg-cover bg-center"
                    style={{ backgroundImage: `url('https://i.pravatar.cc/100?img=${i + 10}')` }}
                  />
                ))}
              </div>
              <p className="text-sm font-semibold tracking-wide uppercase">
                <span className="text-primary">4,287</span> Cars Repaired
              </p>
            </div>
          </div>
        </div>
